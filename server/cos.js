...
const missingCosFields = ['SecretId', 'SecretKey', 'Bucket', 'Region']
  .filter((key) => !COS_CONFIG[key]);

if (missingCosFields.length) {
  throw new Error(
    `缺少 COS 配置: ${missingCosFields.join(', ')}。请在 .env/.env.local 中填写 COS_SECRET_ID、COS_SECRET_KEY、COS_BUCKET、COS_REGION。`
  );
}

// 若未显式提供自定义域名，按桶与地域推导默认访问域
 ...
 */
export async function generateUniqueFileNameFromSoftwareName(softwareName, ext) {
