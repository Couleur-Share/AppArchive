...
const rateLimitKey = (req) => (req.userId || req.ip || "anonymous").toString();
...
app.post("/api/ai/analyze", requireAuth, aiRateLimiter, async (req, res) => {
	...
	try {
		...
		const { data } = await runSoftwareAnalyze(software);
		...
		const duration = Date.now() - startedAt;
		console.log(`[AI_ANALYZE_METRIC] user=${userId} duration=${duration}ms`);
	}
});
...
app.post("/api/ai/config/test", requireAuth, async (req, res) => {
	try {
		...
		res
			.status(400)
			.json({ success: false, error: "测试失败", message: error.message });
	}
});
...
app.get("/api/software/:id", async (req, res) => {
	try {
		const { id } = req.params;
		const result = await pool.query("SELECT * FROM softwares WHERE id = $1", [
			id,
		]);
...
app.post("/api/software", requireAuth, writeRateLimiter, async (req, res) => {
	try {
		...
		// 结构化字段的类型防御：非字符串降为空串，非数组降为空数组
		const safeTagline = typeof tagline === "string" ? tagline.trim() : "";
		const safeHighlights = Array.isArray(highlights)
			? highlights.filter((h) => h && typeof h === "object")
			: [];
		const safeBestFor = Array.isArray(best_for)
			? best_for.filter((h) => h && typeof h === "object")
			: [];
		const safeAvoidIf = Array.isArray(avoid_if)
			? avoid_if.filter((h) => h && typeof h === "object")
			: [];
    ...
    `;
		const values = [
			...
			JSON.stringify(safeBestFor),
			JSON.stringify(safeAvoidIf),
		];
...
app.put(
	...
	async (req, res) => {
		try {
			...
			// 结构化字段归一化：非字符串降为空串，非数组降为空数组
			if (Object.hasOwn(updateData, "tagline")) {
				const v = updateData.tagline;
				updateData.tagline = typeof v === "string" ? v.trim() : "";
			}
			if (Object.hasOwn(updateData, "highlights")) {
				const v = updateData.highlights;
				updateData.highlights = Array.isArray(v)
					? v.filter((h) => h && typeof h === "object")
					: [];
			}
			if (Object.hasOwn(updateData, "best_for")) {
				const v = updateData.best_for;
				updateData.best_for = Array.isArray(v)
					? v.filter((h) => h && typeof h === "object")
					: [];
			}
			if (Object.hasOwn(updateData, "avoid_if")) {
				const v = updateData.avoid_if;
				updateData.avoid_if = Array.isArray(v)
					? v.filter((h) => h && typeof h === "object")
					: [];
			}
