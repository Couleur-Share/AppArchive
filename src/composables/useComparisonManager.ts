...
export function useComparisonManager(
	...
	const toggleComparison = async (software: Software) => {
		try {
			...
			if (existing) {
				...
				if (groupId) {
					await comparisonService.removeSoftwareFromGroup(
						groupId,
						existing.target_id,
					);
					selectedComparisons.value = selectedComparisons.value.filter(
						(c) => c.id !== existing.id,
					);
	...
	const isSelected = (id: number) =>
		selectedComparisons.value.some((c) => c.target_id === id);
