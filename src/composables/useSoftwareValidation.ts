...
export const softwareFormSchema = z.object({
  ...
  icon: z
    ...
    .refine((v) => {
       ...
       if (typeof v === 'string') {
          return v.startsWith('https://') && v.includes('cos.') && v.includes('myqcloud.com')
       }
       return true
    }, '图标必须上传到腾讯云COS'),
  license: z
...
export function useSoftwareValidation(
  ...
  const validateAll = () => {
    ...
    if (data.icon instanceof File) {
        ...
        else if (file.size > 1024 * 1024) errors.value.icon = '图片过大，请控制在 1MB 以内'
    }
    ...
    if (Array.isArray(data.secrets)) {
        data.secrets.forEach((sec, idx) => {
            ...
            const valueFilled = typeof sec?.value === 'string' && sec.value.trim().length > 0
