...
export const comparisonService = {
	...
	async getComparisons(
		...
		try {
			// 获取包含该软件的所有比较组
			const response = await apiRequest(
				`/comparison/software/${softwareId}/groups`,
			);
			return response.data || [];
		} catch (error) {
	...
	async getComparisonSummary(
		...
		try {
			const response = await apiRequest(
				`/comparison/groups/${groupId}/summary`,
			);
			return response.data;
		} catch (_error) {
			return null;
