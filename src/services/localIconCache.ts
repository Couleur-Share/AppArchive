...
export const getIconUrl = (iconPath: string | null | undefined): string => {
  ...
  if (iconPath.startsWith('https://') || iconPath.startsWith('http://')) {
    // 验证是否为COS URL
    if (iconPath.includes('cos.ap-guangzhou.myqcloud.com') || 
 ...
 * @returns 加载状态
 */
export const getIconLoadStatus = (iconPath: string): 'loaded' | 'error' | 'pending' => {
  const status = iconCache.value.get(iconPath)
  return status === 'loaded' ? 'loaded' : status === 'error' ? 'error' : 'pending'
}
