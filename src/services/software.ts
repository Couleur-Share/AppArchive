import type { Software, ApiResponse, PaginatedResponse } from '../types'
...
export const softwareService = {
  ...
  async addSoftware(data: Partial<Software>) {
    try {
      const softwareData = {
        ...
        download_links: Array.isArray((data as any).download_links) ? (data as any).download_links : undefined,
        secrets: Array.isArray((data as any).secrets) ? (data as any).secrets : undefined
      }
  ...
  async updateSoftware(id: number, data: Partial<Software>) {
    try {
      // 确保 pros 和 cons 是数组
      const { id: _, created_at, ...updateData } = {
        ...data,
        pros: Array.isArray(data.pros) ? data.pros : [],
        cons: Array.isArray(data.cons) ? data.cons : [],
        download_links: Array.isArray((data as any).download_links) ? (data as any).download_links : undefined,
      ...
      const sanitized = Object.fromEntries(Object.entries(updateData).filter(([_, v]) => v !== undefined))
  ...
  async getSoftwareByCategory(category: string) {
    try {
      const result = await apiRequest(`/software/category/${encodeURIComponent(category)}`)
      return result.data as Software[]
    } catch (error) {
      throw errorHandler.handle(error)
    }
  ...
  async searchSoftware(query: string) {
    try {
      const result = await apiRequest(`/software/search/${encodeURIComponent(query)}`)
      return result.data as Software[]
    } catch (error) {
      throw errorHandler.handle(error)
    }
