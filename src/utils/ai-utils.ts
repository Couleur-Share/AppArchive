...
export function extractProsConsFromContent(content: string): { description?: string; pros: string[]; cons: string[]; systems?: string[] } {
  ...
  const rawJsonCandidate = (rawStart !== -1 && rawEnd !== -1 && rawEnd > rawStart)
  ...
  try {
    ...
    return {
      description,
      pros: pros.filter((x: any) => typeof x === 'string').map((s: string) => s.trim()).filter(Boolean),
      cons: cons.filter((x: any) => typeof x === 'string').map((s: string) => s.trim()).filter(Boolean),
      systems: systems.filter((x: any) => typeof x === 'string').map((s: string) => s.trim()).filter(Boolean)
    }
  } catch {}
