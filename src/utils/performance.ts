...
// 通过环境变量控制是否输出性能相关日志
const ENABLE_PERFORMANCE_LOGS = import.meta.env.VITE_ENABLE_PERF_LOGS === 'true'
...
export const memoryMonitor = {
  logMemoryUsage() {
    if ('memory' in performance) {
      ...
      if (ENABLE_PERFORMANCE_LOGS) logger.debug('内存使用情况：', {
...
export const domPerformanceMonitor = {
  observeElementCount() {
    ...
    if (elementCount > 1000) {
      if (ENABLE_PERFORMANCE_LOGS) logger.warn('⚠️ DOM 元素过多，建议启用虚拟滚动')
    }
  },
  ...
  observeImageLoading() {
    ...
    if (ENABLE_PERFORMANCE_LOGS) logger.debug(`图片加载情况: ${loadedCount}/${images.length}`)
  }
}
...
export const devPerformanceTips = {
  checkForCommonIssues() {
    ...
    if (blurElements.length > 5) {
      if (ENABLE_PERFORMANCE_LOGS) logger.warn('⚠️ 检测到过多的模糊效果，这可能影响滚动性能')
    }
    
    // 检查是否有未优化的图片
    const images = document.querySelectorAll('img:not([loading="lazy"])')
    if (images.length > 3) {
      if (ENABLE_PERFORMANCE_LOGS) logger.warn('⚠️ 检测到未启用懒加载的图片，建议添加 loading="lazy"')
    }
    
    // 检查是否有过多的动画
    const animatedElements = document.querySelectorAll('[class*="transition-all"]')
    if (animatedElements.length > 10) {
      if (ENABLE_PERFORMANCE_LOGS) logger.warn('⚠️ 检测到过多的 transition-all，建议使用具体的过渡属性')
    }
  }
} 